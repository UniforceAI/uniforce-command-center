...
const VisaoGeral = () => {
  ...
  // Top 5 por métrica selecionada (Churn ou Vencido) - MESMA LÓGICA do gráfico
  const top5Risco = useMemo(() => {
    // Usar mesma métrica do gráfico: % interno de cada categoria
    const dataKey = top5Filter === "churn" ? "churnPct" : "financeiroPct";
    
    return [...top5Data]
      .sort((a, b) => ((b as any)[dataKey] || 0) - ((a as any)[dataKey] || 0))
      .slice(0, 5)
      .map(p => ({
        key: p.key,
        label: p.label,
