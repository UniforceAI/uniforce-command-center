...
export function MeusServicosTab() {
  ...
  const { data: subData, isLoading: subLoading } = useStripeSubscription(ispId);
  const isAsaasLegacy = subData?.stripe_billing_source === "asaas";
  ...
  const isLoading = servicesLoading || subLoading;
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <Badge
                  variant="outline"
                  className={
                    asaasData?.setup_pending
                      ? "text-xs font-medium bg-orange-500/15 text-orange-700 border-orange-200"
                      : "text-xs font-medium bg-green-500/15 text-green-700 border-green-200"
                  }
                >
                  {asaasData?.setup_pending ? "Setup Pendente" : "Ativo"}
                </Badge>
              </div>
              <p className="text-sm text-muted-foreground">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {asaasSub.next_due_date && (
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span>Próximo vencimento: <span className="text-foreground font-medium">{formatDate(asaasSub.next_due_date)}</span></span>
                  </div>
                )}
              ...
              {canManage && (
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="gap-2 text-destructive hover:text-destructive hover:bg-destructive/10"
                    onClick={() => setAsaasCancelOpen(true)}
                  >
                    Cancelar Assinatura
                  </Button>
                </div>
